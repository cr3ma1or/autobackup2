...
- Процессы:
  - Сервис 3x-ui (Web UI: порт `<example>39284</example>`, Подписки: порт `<example>39285</example>`, Reality: порт `443`).
...
- Процессы:
  ...
  - Локальный сервис 3x-ui (Web UI, подписки и Reality); номера портов определяются локальной конфигурацией узла.
...
- **Telegram Bot API:** Отправка уведомлений о backup, failover и результате синхронизации (успехе или ошибке) в Python workflow.
