...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "validate":
            ...
            payload = {"valid": True}
            print(json.dumps(payload) if args.json else "valid")
            return 0
        if args.command == "check-backup":
            ...
            payload = {"valid": True, "backup": metadata.__dict__}
            print(
                json.dumps(payload, default=str) if args.json else metadata.archive_name
            )
        ...
        if args.command == "rollback":
            ...
            payload = {"success": True, "snapshot": str(args.snapshot)}
            print(json.dumps(payload) if args.json else "rollback complete")
            return 0
