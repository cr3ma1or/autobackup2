...
Синхронизация выполняется каждые 12 часов — в `04:00` и `16:00 UTC` — со случайным смещением до 15 минут. На Secondary также устанавливается `xui-backup.timer`; в режиме `STANDBY` он остановлен и отключён. Команда `xui-failover promote` включает его для резервного копирования данных, появившихся на активированном Secondary. До аварии подготовьте отдельные `/etc/x-ui/.env`, `primary-local-gnupg` и GPG recipients для DR-backup: installer не создаёт эти секреты автоматически.
