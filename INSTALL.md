...
Синхронизация выполняется каждые 12 часов — в `04:00` и `16:00 UTC` — со случайным смещением до 15 минут. На Secondary также устанавливается `xui-backup.timer`, но он намеренно оставляется остановленным и отключённым в режиме `STANDBY` **и после `promote`**. До его ручного включения на promoted узле необходимо provision-ить отдельные `/etc/x-ui/.env`, `primary-local-gnupg` и GPG recipients для DR-backup; installer не создаёт эти секреты автоматически.
